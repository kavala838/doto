...
const NewGoalModal: React.FC<NewGoalModalProps> = ({ onClose }) => {
  ...
  const { addGoal, data } = useAppContext();
  ...
  const handleCreateGoal = (useEnhanced = true) => {
    if (!title || (!useEnhanced && !description) || (useEnhanced && !enhancedDescription)) return;
    
    // Generate unique ID using current datetime including seconds
    const timestamp = Date.now();
    const newId = `goal_${timestamp}`;
    
    console.log("Creating new goal", { 
      id: newId, 
      title, 
      tags: selectedTags,
      hasTimeframe: useTimeframe
    });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ marginTop: '0', marginBottom: '20px' }}>Create New Goal</h2>
          ...
          <>
            <div style={{ marginBottom: '15px' }}>
              ...
              <small style={{ color: '#666', marginTop: '5px', display: 'block' }}>
                You can enhance this description with AI to include Summary and Topics sections, or create the goal with the description as-is.
              </small>
            </div>
